...
def test_load_config_with_old_config_conversion():
    old_config_data = {
        "actions": [
            ...
            {"button_id": 2, "action": "send-reaction", "extra": "like"},
        ],
        "longpress_action": [],
        "leds": [],
        "teams_token": None,
    ...
    converted_config = Config(**old_config_data)
    with (
        patch("pathlib.Path.exists", return_value=True),
        patch("builtins.open", mock_open(read_data=yaml.dump(old_config_data))),
        patch("yaml.safe_load", return_value=old_config_data),
        patch(
            "mutenix.utils.config_converter.convert_old_config",
            return_value=converted_config,
        ),
        patch("mutenix.config.save_config") as mock_save_config,
    ):
        config = load_config()
        assert config.model_dump() == converted_config.model_dump()
